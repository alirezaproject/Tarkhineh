...
import 'package:tarkhineh/features/auth/models/jwt_response.dart';

abstract class IAuthService {
  Future<ApiResult<void>> sendOtp(String phone);
  Future<ApiResult<JwtResponse>> verifyOtp(String phoneNumber, String otpCode);
}
...
class AuthService implements IAuthService {
  ...
  @override
  Future<ApiResult<JwtResponse>> verifyOtp(String phoneNumber, String otpCode) async {
    try {
      final response = await dio.post(ApiEndpoints.verifyOtp, data: {'mobile': phoneNumber, 'code': otpCode});
      return ApiResult.fromJson(response.data, (j) => JwtResponse.fromJson(j));
    } on DioException catch (e) {
      if (e.error is NetworkException) {
        return ApiResult(success: false, message: (e.error as NetworkException).message);
      } else {
